...
class MsspOp {
  public:
    ...
    static __device__ inline MyInt4 apply(volatile MyInt4& t1, volatile MyInt4& t2) {
		// x = t1 y = t2
        int mss = max(t1.x, max(t2.x, t1.z + t2.y)); // ... fill in the blanks ...
        int mis = max(t1.y, t1.w+t2.y); // ... fill in the blanks ...
        int mcs = max(t2.z, t1.z+t2.w); // ... fill in the blanks ...
...
shiftRightByOne(T* d_in, T* d_out, T ne, unsigned int d_size) {
	...
	else if(gid < d_size - 1) {
		d_out[gid] = d_in[gid + 1];
	}
...
msspTrivialMap(int* inp_d, MyInt4* inp_lift, int inp_size) {
    ...
    if(gid < inp_size) {
        if(inp_d[gid] > 0){
			inp_lift[gid] = MyInt4(inp_d[gid], inp_d[gid], inp_d[gid], inp_d[gid]);
		}
		else{
			inp_lift[gid] = MyInt4(0,0,0,inp_d[gid]);
		}
    }
}
...
write_lastSgmElem(float* tmp_scan, int* tmp_inds, int* flags_d, int tot_size, float* vct_res) {
    ...
    if(gid < tot_size) {
		if(gid > 0){		
			if(flags_d[gid] == 1){
				vct_res[tmp_inds[gid-1]] = tmp_scan[gid];//tmp_scan[gid-1];
			}
		}
		else{
			vct_res[0] = tmp_scan[gid];
		}
    }
}

/** Calculate the shp_inds array.
* rowIota	an array of idxs 0....n where n = number of rows
* shp_sc	the scanned array of row lengths
* shp_inds	the result array
**/
__global__ void
mapShpInds(int num_rows, int* rowIota, int* shp_sc, int* shp_inds) {
	const unsigned int gid = blockIdx.x*blockDim.x + threadIdx.x;
	
	if(gid < num_rows){
		if(rowIota[gid] > 0){
			shp_inds[gid] = shp_sc[gid-1];
		}
		else{
			shp_inds[gid] = 0;
		}
...
* num_elms	the total number of elements in the array corresponding to the flag array.
* shp_inds	the inds array of the sparse matrix shp array.
* flags		the output array filled with zeroes
**/
__global__ void
scatterFlags(int num_elms, int* shp_inds, int* flags){
	const unsigned int gid = blockIdx.x*blockDim.x + threadIdx.x;
	if(gid < num_elms){
		flags[shp_inds[gid]] = 1;
	}
}
