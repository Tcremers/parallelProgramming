...
bool validateProgram(float* A, float* B, unsigned int N){
  ...
  for(unsigned int i = 0; i < N; i++) {
    ...
    float tmpB = A[i*64];
    tmpB = tmpB*tmpB;
    if(fabs(B[ii] - tmpB)> 0.00001) { printf("fail1: %f", tmpB); printf("fail1: %f", B[ii]); printf("\n foo is: %u",ii); valid = false; break; }
    for(int j = 1; j < 64; j++) {
        float tmpA  = A[ii + j];
        float accum = sqrt(tmpB) + tmpA*tmpA;

        if(fabs(B[ii+j] - accum) > 0.00001) {printf("fail2"); valid = false; break; }
        tmpB        = accum;
    }
    if(!valid) break;
  }
